...
{
    public class PsychicRitualToil_ProliferationPulse : PsychicRitualToil
    {
        private PsychicRitualRoleDef invokerRole;

        protected PsychicRitualToil_ProliferationPulse()
        {
        }

        public PsychicRitualToil_ProliferationPulse(PsychicRitualRoleDef invokerRole)
        {
        ...
        public override void Start(PsychicRitual psychicRitual, PsychicRitualGraph parent)
        {
            ...
            float duration = ((PsychicRitualDef_ProliferationPulse)psychicRitual.def).durationDaysFromQualityCurve.Evaluate(psychicRitual.PowerPercent);
        ...
        private void ApplyOutcome(PsychicRitual psychicRitual, Pawn invoker, float duration)
        {
            foreach (Pawn pawn in invoker.Map.mapPawns.AllHumanlikeSpawned)
            {
                if ((double)pawn.GetStatValue(StatDefOf.PsychicSensitivity) > 0.0)
                {
                    ...
                    Hediff hediff = HediffMaker.MakeHediff(InternalDefOf.ProliferationPulse, pawn);
            ...
            Find.LetterStack.ReceiveLetter("PsychicRitualCompleteLabel".Translate((NamedArgument)psychicRitual.def.label), "ProliferationPulseCompleteText".Translate((NamedArgument)(Thing)invoker, (NamedArgument)Mathf.FloorToInt(duration * 60000f).ToStringTicksToDays(), psychicRitual.def.Named("RITUAL")), LetterDefOf.NeutralEvent);
        }
